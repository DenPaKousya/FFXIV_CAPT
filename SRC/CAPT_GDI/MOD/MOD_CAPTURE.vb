...
#Region "キャプチャ用持ち回り"
    Private rctPROCESS_CLIENT As RECT
...
#Region "内部メソッド"
    ...
    Private Sub SUB_CAPTURE_MAIN()
        ...
        If Not (srtSAVE.IMAGE Is Nothing) Then
            ...
            Else
                ...
                If Not srtSAVE.IMAGE Is Nothing Then
                    Call GC.ReRegisterForFinalize(srtSAVE.IMAGE)
                End If
                srtSAVE.IMAGE = Nothing
                Call GC.ReRegisterForFinalize(srtSAVE)
                srtSAVE = Nothing
                Call stwTIME.Stop()
    ...
    Private Function FUNC_GET_JPEG_IMAGE(ByRef bmpBASE As Bitmap) As Bitmap
        ...
        bmpRET = bmpBASE.GetThumbnailImage(intWIDTH, intHEIGHT, New Image.GetThumbnailImageAbort(AddressOf Dummy), IntPtr.Zero)
