Friend Module MOD_LOGO

#Region "モジュール用変数"
    Friend FRM_PARENT As FRM_MAIN
    Private WPF_MY_WINDOW As WPF_LOGO
#End Region

#Region "PUBLIC"
    Public Sub SUB_CALL_LOGO(ByRef FRM_CALL As FRM_MAIN)
        If FUNC_CHECK_WINDOW() Then
            Call SUB_CLOSE_LOGO()
        Else
            Call SUB_SHOW_LOGO(FRM_CALL)
        End If
    End Sub

    Public Sub SUB_RECALL_LOGO(ByRef FRM_CALL As FRM_MAIN)
    ...
    Public Sub SUB_SHOW_LOGO(ByRef FRM_CALL As FRM_MAIN)
        ...
        WPF_MY_WINDOW = New WPF_LOGO

        Call SUB_SET_LOCATION_OVERLAY_WPF(WPF_MY_WINDOW, ENM_POSITION_OVERLAY.LEFT_TOP)
        Call WPF_MY_WINDOW.SUB_CHANGE_DRAW_COMPOTION()
        WPF_MY_WINDOW.Background = New SolidColorBrush(Windows.Media.Color.FromArgb(0, 0, 0, 255))
    ...
    Public Function FUNC_GET_BITMAP_LOGO() As Bitmap
        If Not FUNC_CHECK_WINDOW() Then
            Return Nothing
        End If

        Return WPF_MY_WINDOW.BMP_VIEW_LOGO
    End Function

    Public Function FUNC_GET_SIZE_LOGO(ByVal BASE As RECT_WH) As RECT_WH
    ...
    Public Sub SUB_SET_TOPMOST_LOGO(ByVal BLN_TOPMOST As Boolean, ByVal BLN_ACTIVATE As Boolean)
    ...
    Public Function FUNC_GET_HANDLE_LOGO() As IntPtr
        If WPF_MY_WINDOW Is Nothing Then
            Return IntPtr.Zero
        End If
...
#Region "FRIEND"
    Friend Function FUNC_CHECK_WINDOW_LOGO()
        Return FUNC_CHECK_WINDOW()
    End Function

    Friend Function FUNC_GET_SIZE_WINDOW_LOGO() As RECT_WH
